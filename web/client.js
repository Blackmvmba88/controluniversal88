...
(function () {
  ...
  function attemptReconnect() {
    ...
    console.log(
      `Intentando reconexión ${reconnectAttempts}/${MAX_RECONNECT_ATTEMPTS} en ${delay}ms...`
    );
  ...
  function showConnectionStatus(type, message) {
    ...
    if (!statusEl) {
      ...
      statusEl.style.cssText =
        'position:fixed;top:10px;right:10px;padding:10px;border-radius:4px;z-index:9999;font-size:14px;';
  ...
  if (btnStart)
    btnStart.addEventListener('click', async () => {
      ...
      const count = Number(inCount.value) || 3;
  ...
  if (btnAuto)
    btnAuto.addEventListener('click', async () => {
      ...
      const log = document.getElementById('cal-log');
      try {
        const r = await fetch('/api/collect/auto', {
          method: 'POST',
          headers: { 'content-type': 'application/json' },
          body: JSON.stringify({ count, save: false }),
        });
        const j = await r.json();
  ...
  // Remote bookmarklet helpers
  const remoteUrlInput = document.getElementById('remote-url');
  const remoteGenBtn = document.getElementById('remote-gen');
  const remoteTestBtn = document.getElementById('remote-test');
  const remoteStatus = document.getElementById('remote-status');
  // restore saved value
  try {
    const sv = localStorage.getItem('remote_ws_url');
    if (sv) remoteUrlInput.value = sv;
  } catch (e) {}

  if (remoteGenBtn)
    remoteGenBtn.addEventListener('click', async () => {
      const url = remoteUrlInput.value.trim();
      if (!url) {
        alert('Please enter a remote WS URL (wss://...)');
        return;
      }
      ...
      // show it for copy/paste in the panel
      const box = document.getElementById('bookmarklet-snippet');
      const text = document.getElementById('bookmarklet-text');
      if (box && text) {
        box.style.display = 'block';
        text.innerText = bookmarklet;
      }
      try {
        localStorage.setItem('remote_ws_url', url);
      } catch (e) {}

      // If auto-inject is enabled, open/attach to target and inject the bookmarklet
      try {
        const auto =
          document.getElementById('auto-inject') && document.getElementById('auto-inject').checked;
        const target =
          document.getElementById('inject-target') &&
          document.getElementById('inject-target').value;
        if (auto && target) {
          // prefer navigating an existing window; open a new one if not opened
          let w = window._injectedWindow;
          try {
            if (!w || w.closed) w = window.open(target, '_blank');
            // insert script tag into the opened window (same-origin only)
            const sScript = `var s=document.createElement('script');s.src='${location.origin}/bookmarklet.js';document.head.appendChild(s);setTimeout(function(){connectToController('${url}')},200);`;
            // attempt to eval in that window
            setTimeout(() => {
              try {
                w.eval(sScript);
                console.log('Auto-inject attempted');
              } catch (e) {
                console.warn('Auto-inject failed (cross-origin?)', e);
              }
            }, 400);
            window._injectedWindow = w;
          } catch (e) {
            console.warn('Auto-inject flow failed', e);
          }
        }
      } catch (e) {
        console.warn('Auto-inject error', e);
      }
    });
  ...
  if (copyBtn)
    copyBtn.addEventListener('click', async () => {
      const txtEl = document.getElementById('bookmarklet-text');
      if (!txtEl) return;
  ...
  const downloadBtn = document.getElementById('bookmarklet-download');
  if (downloadBtn)
  ...
  if (remoteTestBtn)
    remoteTestBtn.addEventListener('click', async () => {
      const url = remoteUrlInput.value.trim();
      if (!url) {
        remoteStatus.innerText = 'enter URL first';
        return;
      }
      ...
      try {
        ...
        const to = setTimeout(() => {
          ...
          remoteStatus.innerText = 'timeout';
        }, 3000);
        ws.addEventListener('open', () => {
          clearTimeout(to);
  ...
  async function showModal(label) {
    ...
    [modalAccept, modalRetry, modalSkip, modalClose].forEach((b) => {
      b.style.display = '';
    });
    ...
    function onKey(e) {
      ...
      if (e.key === 'Tab') {
        ...
        else next = (idx + 1) % focusable.length;
    ...
    function cleanup() {
      ...
      modalRetry.removeEventListener('click', onRetry);
      modalSkip.removeEventListener('click', onSkip);
        ...
        while (true) {
          try {
            ...
            const r = await fetch('/api/collect/start', {
              method: 'POST',
              headers: { 'content-type': 'application/json' },
              body: JSON.stringify({ label: lbl, count, save: false }),
            });
            const j = await r.json();
            ...
            if (!job) {
              log.innerText = `Timeout waiting for ${lbl}`;
              if (!confirm(`Timeout for ${lbl}. Continue?`)) throw new Error('user aborted');
              else break;
            }
            ...
            if (action === 'accept') {
              ...
              try {
                ...
                log.innerText = `Saved mapping for ${lbl}`;
                break;
              } catch (e) {
                log.innerText = `Failed to save mapping: ${e}`;
                if (!confirm('Retry save?')) break;
              }
            } else if (action === 'retry') {
              log.innerText = `Retrying ${lbl}...`;
              continue;
            } else if (action === 'skip' || action === 'close') {
              log.innerText = `Skipping ${lbl}`;
              break;
            }
          } catch (e) {
            log.innerText = `Error collecting ${lbl}: ${e}`;
            if (!confirm('Continue with next?')) {
              break;
            } else {
              break;
            }
