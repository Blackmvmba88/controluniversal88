...
function testSanitizeLabel() {
  ...
  if (v('cross;drop table') !== 'crossdroptable')
    throw new Error('Expected sanitization of SQL injection attempt');
...
function testValidateMapping() {
  ...
  if (!r2.errors.some((e) => e.includes('cross')))
    throw new Error('Expected error about cross button');
...
function testValidateAndSanitizeCollectParams() {
  ...
  if (!noLabel.errors.some((e) => e.includes('requerido')))
    throw new Error('Expected error about required label');
  ...
  if (!invalidCount.errors.some((e) => e.includes('count')))
    throw new Error('Expected error about count');
