...
async function testCollectLibValidation(){
  ...
  try{ if (fs.existsSync('.ds4map.samples.json')) fs.unlinkSync('.ds4map.samples.json'); } catch(e){}
  // collect simulated samples for label 'cross'
  const mapping = await collectLib.collectSamples({ label: 'cross', count: 3, simulate: true, save: false }, (s)=>{});
  ...
  if (!mapping._validation.ok) throw new Error('validation failed: ' + JSON.stringify(mapping._validation.details));
}
...
async function testCollectAutoSaveEndpoint(){
  ...
  try{ if (fs.existsSync('.ds4map.json')) fs.unlinkSync('.ds4map.json'); } catch(e){}
  ...
  if (!job.validation.ok) throw new Error('validation failed for auto job: ' + JSON.stringify(job.validation.details));
