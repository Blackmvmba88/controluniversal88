...
async function testGuidedApiFlow(){
  ...
  // wait done
  for (let i=0;i<40;i++){ await new Promise(r=>setTimeout(r,200)); const s = await getStatus(); if (s && s.status === 'done') break; }
  let s1 = await getStatus(); if (!s1.result || !s1.result.buttons) throw new Error('no result for cross');

  await postStart('circle', 1, false);
  for (let i=0;i<40;i++){ await new Promise(r=>setTimeout(r,200)); const s = await getStatus(); if (s && s.status === 'done') break; }
  let s2 = await getStatus(); if (!s2.result || !s2.result.buttons) throw new Error('no result for circle');
