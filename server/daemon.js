...
class Daemon extends EventEmitter {
   ...
   *
   * @param {Object} [mapping] - Opcional: mapeo inicial (si no se provee, carga de disco)
   */
  constructor(mapping) {
    super();
    this.mapping = mapping || this._loadMap();
    this.prevState = null;
    this._device = null;
  ...
  _updateCaches() {
    this._axesCache =
      this.mapping && this.mapping.axes && typeof this.mapping.axes === 'object'
        ? this.mapping.axes
        : {};
    this._buttonsCache =
      this.mapping && this.mapping.buttons && typeof this.mapping.buttons === 'object'
  ...
  /**
   * Reinicia el estado del daemon (útil para tests)
   */
  reset() {
    this.prevState = null;
    this._recent = [];
  ...
  start() {
    if (!SIMULATE && HID) {
      ...
      logger.info(
        'Daemon: ejecutando en modo SIMULACIÓN (establecer SIMULATE=0 para usar node-hid)'
      );
      this._simulate();
    }
  }
  ...
  _tryOpenDevice() {
    try {
      ...
      const ds = devices.find((d) => {
        ...
        return /Sony|PlayStation|Wireless Controller|PowerA|XBX|Xbox|Controller/i.test(
          combinedString
        );
      });
  ...
  _simulate() {
    ...
    const buttons = this.mapping && this.mapping.buttons ? Object.keys(this.mapping.buttons) : [];
