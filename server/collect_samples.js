...
async function collect(label, count = 3, timeout = 8000){
  console.log(`Collecting ${count} sample pairs for '${label}'. Please be ready to press/release the control when prompted.`);
  const devices = HID ? HID.devices() : [];
  const target = devices.find(d => /Sony/i.test(d.manufacturer || '') || /Wireless Controller/i.test(d.product || ''));
  if (!target) console.warn('Warning: no HID device found — you can still run this on a device by connecting it.');

  let device = null;
  if (target){
    try { device = new HID.HID(target.path); device.setNonBlocking && device.setNonBlocking(true); }
    catch(e){ console.warn('Failed to open HID device:', e); device = null; }
  }
  ...
  if (process.env.SIMULATE === '1'){
    ...
    await collectLib.collectSamples({ label, count, timeout, simulate: true }, (s)=>{ console.log('progress', s); });
  ...
  for (let i=0;i<count;i++){
    await prompt(`Press the control for sample ${i+1} (then hit ENTER to capture 'after' report)`);
    try{
  ...
  let existing = [];
  try { if (fs.existsSync(samplesPath)) existing = JSON.parse(fs.readFileSync(samplesPath,'utf8')); } catch(e){}
  existing.push(...pairs);
  ...
  const mapping = core.inferMappingsFromLabeledReports(existing.map(p => ({ label: p.label, before: p.before, after: p.after })));
  ...
  fs.writeFileSync(outPath, JSON.stringify({ axes: {}, buttons: mapping, dpad: { byte: null, mask: 15 } }, null, 2));
...
async function main(){
  ...
  if (!label) { console.error('Usage: node server/collect_samples.js --label <name> [--count N]'); process.exit(2); }
