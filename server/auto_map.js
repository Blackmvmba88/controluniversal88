...
async function run(){
  ...
  if (!target) console.log('No DualShock-like device found; please connect device or run in SIMULATE=1 and press keys on your controller emulator');

  let device = null;
  if (target){
    try { device = new HID.HID(target.path); console.log('Opened HID', target.product || target.path); }
    catch(e){ console.error('Failed to open HID device', e); device = null; }
  }
  ...
  const readBuf = (buf) => {
    ...
    if (state.raw) state.raw = arr; else state.raw = arr;
  ...
  function waitDiff(timeout = 8000){
    return new Promise((resolve, reject) => {
      ...
      const timer = setTimeout(()=>{ listeners = listeners.filter(f=>f!==onDiff); reject(new Error('timeout')); }, timeout);
      function onDiff(arr){
        const diffs = printDiff(st || [], arr);
        if (diffs.length){ clearTimeout(timer); listeners = listeners.filter(f=>f!==onDiff); resolve(diffs); }
      }
      listeners.push(onDiff);
    });
  }

  console.log('Auto-mapping will now prompt for each button. For each prompt, press and release the specified button once.');

  // Collect multiple attempts per button and use consensus inference
  const collected = {};
  for (const btn of BUTTONS){
    await prompt(`Ready for button \'${btn}\' — press it now then hit ENTER when you pressed it (or wait to timeout)...`);
    try{
  ...
  for (const d of dpadDirs){
    ...
    } catch(e){ console.log('Timeout mapping', d); }
  }
  ...
  for (const ax of AXES){
    ...
    } catch(e){ console.log('Timeout mapping axis', ax); }
  }
  ...
  try {
    ...
    utils.saveMappingWithBackup({ axes: mapping.axes || {}, buttons: mapping.buttons || {}, dpad: mapping.dpad || { byte: null, mask: 15 } }, outPath);
    console.log('Saved mapping to', outPath);
  } catch (e) { console.error('Failed to save mapping:', e); }
