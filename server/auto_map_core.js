...
function validateMapping(mapping, labeledPairs) {
  ...
  const details = [];
  let ok = true;
  const buttons = mapping.buttons && typeof mapping.buttons === 'object' ? mapping.buttons : {};
  ...
  for (const p of labeledPairs) {
    ...
    const before = idx < p.before.length && typeof p.before[idx] === 'number' ? p.before[idx] : 0;
    const after = idx < p.after.length && typeof p.after[idx] === 'number' ? p.after[idx] : 0;
    ...
    for (const [otherBtn, otherM] of Object.entries(buttons)) {
      ...
      const obefore =
        oIdx < p.before.length && typeof p.before[oIdx] === 'number' ? p.before[oIdx] : 0;
      const oafter = oIdx < p.after.length && typeof p.after[oIdx] === 'number' ? p.after[oIdx] : 0;
