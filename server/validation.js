...
function validateMapping(mapping) {
    ...
    if (typeof mapping.buttons !== 'object' || Array.isArray(mapping.buttons)) {
      ...
      for (const [name, pair] of Object.entries(mapping.buttons)) {
        if (!Array.isArray(pair) || pair.length !== 2) {
          errors.push(
            `mapping.buttons["${name}"] debe ser un array de 2 elementos [byteIdx, mask]`
          );
        } else {
          const [idx, mask] = pair;
          if (typeof idx !== 'number' || idx < 0 || !isFinite(idx)) {
            errors.push(
              `mapping.buttons["${name}"][0] (byteIdx) debe ser un número positivo finito`
            );
          }
  ...
  if (mapping.dpad !== undefined) {
    if (typeof mapping.dpad !== 'object' || Array.isArray(mapping.dpad)) {
      ...
      if (mapping.dpad.byte !== undefined && mapping.dpad.byte !== null) {
        if (
          typeof mapping.dpad.byte !== 'number' ||
          mapping.dpad.byte < 0 ||
          !isFinite(mapping.dpad.byte)
        ) {
          errors.push('mapping.dpad.byte debe ser un número positivo finito');
        }
      }
      if (mapping.dpad.mask !== undefined && mapping.dpad.mask !== null) {
        if (
          typeof mapping.dpad.mask !== 'number' ||
          mapping.dpad.mask < 0 ||
          !isFinite(mapping.dpad.mask)
        ) {
